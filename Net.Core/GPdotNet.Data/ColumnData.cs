...
namespace GPdotNet.Data
{
    ...
    public class ColumnData
    {
        ...
        static string[]                 m_missingSymbols = { "n/a", " ", "?", ""};     
        ...
        {
            m_Encoding = CategoryEncoding.None;
            if (isOutput)
       ...
       /// <returns></returns>
        ...
        public string EncodedHeader(int index)
        {
            ...
            else if (ColumnDataType == ColumnType.Category && CategoryEncoding.OnevsAll == Encoding)
            {
                return $"{Name}({m_Statistics.Categories[index]})";
            }
            else if (ColumnDataType == ColumnType.Category && CategoryEncoding.OnevsAll_1 == Encoding)
            {
            ...
            for (int i = 0; i < m_RealValues.Length; i++)
            {
                ...
                if (m_scalingType == Scaling.Gauss)
                {
                    if (m_Statistics.StdDev == 0)
                        ...
                        m_EncodedValues[i][0] = (m_NumericValues[i] - m_Statistics.Mean) / m_Statistics.StdDev;
                }
                else if (m_scalingType == Scaling.MinMax)
                {
                    if (m_Statistics.Max - m_Statistics.Min==0)
                    {
                      ...
                      m_EncodedValues[i][0] = (m_NumericValues[i] - m_Statistics.Min) / (m_Statistics.Max - m_Statistics.Min);
                }                  
                else if (m_scalingType == Scaling.None)
                    m_EncodedValues[i][0] = m_NumericValues[i];
                else
        ...
        private void RealDataToCategoric(Statistics stat = null)
        {
            ...
            {
                //check if test data contains new classes
                var testClasses = m_RealValues.Where(x => !m_missingSymbols.Contains(x)).Distinct().OrderBy(x => x).ToList();
                foreach(var c in testClasses)
                {
                    if (!stat.Categories.Contains(c))
                        stat.Categories.Add(c);
                }
                classes = stat.Categories;
            }
        ...
        ///          Red   =  (1,0,0)
        ///          Green =  (0,1,0)
        /// Normalized values for Blues category: dummy encoding -N-1
            ...
            for (int i = 0; i < m_RealValues.Length; i++)
            {
                switch (Encoding)
                {
                    ...
                    case CategoryEncoding.Level:
                        ...
                        break;
                    case CategoryEncoding.OnevsAll:
                        {
                        ...
                        break;
                    case CategoryEncoding.OnevsAll_1:
                        {
        ...
        {
            switch (m_MissingValue)
            {
                case MissingValue.Ignore:
                    return "Ignore";
                case MissingValue.Average:
                    return "Average";
                case MissingValue.Random:
                    return "Random";
                case MissingValue.Mode:
                    return "Mode";
                case MissingValue.Max:
                    return "Max";
                case MissingValue.Min:
                    return "Min";
                default:
                    return "Ignore";
            }
