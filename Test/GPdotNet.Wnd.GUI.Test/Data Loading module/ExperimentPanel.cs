...
namespace GPdotNet.Wnd.TestApp
{
    ...
    public partial class ExperimentPanel : UserControl
    {
        ...
        /// <summary>
        /// Parses the ListView control and define columns for Projects data
        /// </summary>
        /// <returns></returns>
        private MetaColumn[] ParseColumns()
        {
            ...
            for (int i=1; i <firstRow.SubItems.Count; i++)
            {
                ...
                lst.Add(new MetaColumn() { Index = colIndex, Name = colName, Encoding= colEncoding,  Type = colType, Param = paramType, Scale = normType, MissingValue = missingValue });
            }
        ...
        private string[][] ParseData()
        {
            ...
            for (int k = 0; k < listView1.Items.Count-6; k++)
            {
                ...
                data[k] = new string[row.SubItems.Count-1];
                for (int j = metadataColumns; j < row.SubItems.Count; j++)
                {
                    data[k][j- metadataColumns] = row.SubItems[j].Text;
                }
            }
        ...
        /// <summary>
        /// Creating Columns from string data
        /// </summary>
        /// <param name="columnCount"></param>
        /// <param name="pstr"></param>
        /// <returns></returns>
        private List<MetaColumn> loadMetaDataColumn(int columnCount, string[] pstr)
        {
            ...
            for (int i = 0; i < columnCount; i++)
            {
                ...
                col.Index = int.Parse(pstr[ind]);
                col.Name = pstr[ind + 1];
                col.Type = pstr[ind + 2];
                col.Param = pstr[ind + 3];
                col.Scale = pstr[ind + 4];
        ...
        public DataSet1 GetData()
        {
            try
            {
                ...
                //
                data1.MetaData = ParseColumns();
                var strData = ParseData();
                if (numberRadio.Checked)
                {
                    
                    int rows = (int)numCtrlNumForTest.Value;
                    if (rows < 0 || rows >= strData.Length)
                        throw new Exception("The number of testing rows exceeded the data set size!");
                    //
                    data1.TestRows = (int)numCtrlNumForTest.Value;
                }
                else
                {
                    int precent = (int)numCtrlNumForTest.Value;
                    if (precent < 0 || precent > 50)
                        throw new Exception("Percentage number must be between 0 and 50!");
                    var totalRows = strData[1].Length;
                    data1.TestRows = (int)Math.Round((totalRows* precent) / 100.0,0);  
                }

                if (randomoizeDataSet.Checked)
                {
                    var rnd = new Random((int)DateTime.UtcNow.Ticks);
                    var data = strData.ToList<string[]>();
                    var res1 = data.OrderBy(row => rnd.Next());
                    var res2 = data.OrderBy(row => rnd.Next());
                    strData = res2.ToArray();
                }
                //
                data1.Data = strData;
                return data1;
        ...
        public void SetData(DataSet1 data)
        {
            //
            randomoizeDataSet.Checked = false;
        ...
        private DataDescriptor loadMetaData()
        {
            ...
            //des.Features = new MetaColumn[5];
            //des.Features[0] = new MetaColumn { Id = 1, Name = "precent", Index = 0, Type = ColumnType.Numeric, DefaultMissingValue = 0.5, Values = null };
            //des.Features[1] = new MetaColumn { Id = 2, Name = "color", Index = 1, Type = ColumnType.CLASS, DefaultMissingValue = 0, Values = new string[3] { "red", "green", "blue" } };
            //des.Features[2] = new MetaColumn { Id = 3, Name = "gender", Index = 2, Type = ColumnType.BINARY, DefaultMissingValue = 1, Values = new string[2] { "male", "female" } };
            //des.Features[3] = new MetaColumn { Id = 4, Name = "year", Index = 3, Type = ColumnType.NUMERIC, DefaultMissingValue = 15, Values = null };
            //des.Features[4] = new MetaColumn { Id = 5, Name = "y", Index = 4, Type = ColumnType.BINARY, DefaultMissingValue = 1, Values = new string[2] { "no", "yes" } };
    ...
    public class DataDescriptor
    {
        public DataDescriptor()
        {
            ...
            //bay default DataDescriptor doesn't contain proper LabelIndex
        ...
        /// </summary>
        public MetaColumn[] Features { get; set; }
